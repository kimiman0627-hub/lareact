        ...
        <Link href={`/post/${post.post_id}`}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4">
                <div className="flex items-center gap-2 mb-1.5">
                    <RankBadge rank={1} />
                    <span className="text-[10px] font-semibold text-white/70 bg-white/15 rounded px-2 py-0.5">{post.board_name}</span>
                </div>
                <p className="text-white font-bold text-sm leading-snug line-clamp-2 group-hover:text-sky-300 transition">{post.title}</p>
                <div className="mt-1.5 flex items-center gap-2 text-[11px] text-white/50">
                    <span>{post.author}</span>
                    <span>·</span>
                    <span>{timeAgo(post.created_at)}</span>
                    {(post.comment_count ?? 0) > 0 && (
                        <><span>·</span><span className="text-sky-400">댓글 {post.comment_count}</span></>
                    )}
                </div>
            </div>
        </Link>
...
function SmallCard({ post, rank }: SmallCardProps) {
    const [imgError, setImgError] = useState(false);
...
export default function PopularWidget({ posts = [] }: PopularWidgetProps) {
    ...
    return (
        <section className="mb-4">
            <div className="flex items-center justify-between mb-3">
                ...
                <Link href="/popular" className="text-xs text-slate-400 hover:text-blue-500 transition font-medium">전체보기 →</Link>
            </div>
