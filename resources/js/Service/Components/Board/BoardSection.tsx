...
function FeaturedPost({ post, grad }: FeaturedPostProps) {
...
function CompactRow({ post, idx }: CompactRowProps) {
    return (
        <li>
            <Link href={`/post/${post.post_id}`}
                className="flex items-center gap-2 px-3 py-2 hover:bg-gray-50 transition group border-b border-gray-50 last:border-0">
                <span className="shrink-0 w-4 text-center text-[10px] font-mono text-slate-300 group-hover:text-blue-400 transition">{idx}</span>
                ...
                <span className="shrink-0 text-[10px] text-slate-300 whitespace-nowrap">{timeAgo(post.created_at)}</span>
            </Link>
        </li>
    );
}
...
export default function BoardSection({ board, colorIndex = 0 }: BoardSectionProps) {
    ...
    return (
        <section className="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-100">
            <div className={`flex items-center justify-between px-3 py-2 border-b ${scheme.hdr}`}>
                ...
                <Link href={href} className="text-[11px] text-slate-400 hover:text-blue-500 transition font-medium">전체보기 →</Link>
            </div>
