...
export const fmtHits = (n: number | undefined): string => {
    if (!n) return "0";
...
const truncate = (str: string | undefined, len: number = 38): string =>
    ...
    return (
        <article className="bg-white rounded border border-gray-200 shadow-sm flex flex-col overflow-hidden">
            ...
            <ul className="flex-1 divide-y divide-gray-100">
                {posts.length === 0 && (
                    <li className="px-3 py-5 text-center text-xs text-slate-400">게시글이 없습니다.</li>
                )}
                {posts.map((post) => (
                    <li key={post.post_id}>
                        <Link href={`/post/${post.post_id}`}
                            ...
                            <span className="flex-1 text-sm text-slate-800 group-hover:text-sky-700 leading-snug min-w-0 truncate">
                                ...
                                {(post.comment_count ?? 0) > 0 && (
                                    <span className="ml-1 text-sky-500 font-bold text-xs">[{post.comment_count}]</span>
                                )}
                            </span>
