...
import type { Post, Comment, User, SeoData, SharedProps } from "@/types";
...
function CommentForm({ postId, parentId = null, onCancel = null, autoFocus = false }: CommentFormProps) {
            ...
            <div className="flex justify-end gap-2 mt-1.5">
                {onCancel && (
                    <button type="button" onClick={onCancel}
                        className="px-3 py-1 text-xs text-slate-500 hover:text-slate-700 transition">취소</button>
                )}
...
function CommentItem({ comment, postId, authUser, maxDepth, replies = [], allComments, depth = 1 }: CommentItemProps) {
        ...
        <li>
            <div className={`flex gap-3 py-3 ${isNested ? "pl-4" : ""}`}>
                ...
                <div className={`flex-1 min-w-0 ${isNested ? "bg-slate-50 rounded-xl px-3 py-2 border border-slate-100" : ""}`}>
                    ...
                    <p className="text-sm text-slate-700 whitespace-pre-wrap wrap-break-word leading-relaxed">{comment.content}</p>
                    ...
                    {showReplyForm && authUser && (
                        <div className="mt-2">
                            <CommentForm postId={postId} parentId={comment.comment_id}
                                onCancel={() => setShowReplyForm(false)} autoFocus />
                        </div>
                    )}
                </div>
            </div>
            ...
            {replies.length > 0 && (
                <ul className="border-l-2 border-blue-200 ml-10 pl-3 mb-1">
                    {replies.map(reply => (
                        <CommentItem key={reply.comment_id} comment={reply} postId={postId}
                            authUser={authUser} maxDepth={maxDepth}
                            replies={allComments.filter(c => c.parent_id === reply.comment_id)}
                            allComments={allComments} depth={depth + 1} />
                    ))}
                </ul>
            )}
        </li>
...
function ShareButton({ title, ogImage }: ShareButtonProps) {
    ...
    function shareKakao() {
        const metaEl = document.querySelector('meta[name="kakao-key"]') as HTMLMetaElement | null;
        const appKey = metaEl?.content;
        ...
        const doShare = () => {
            try {
                ...
                const shareParams = ogImage
                    ? { objectType: "feed", content: { title, description: "", imageUrl: ogImage, link: { mobileWebUrl: currentUrl, webUrl: currentUrl } }, buttons: [{ title: "게시글 보기", link: { mobileWebUrl: currentUrl, webUrl: currentUrl } }] }
                    : { objectType: "feed", content: { title, description: currentUrl, imageUrl: "https://developers.kakao.com/assets/img/about/logos/kakaolink/kakaolink_btn_medium.png", link: { mobileWebUrl: currentUrl, webUrl: currentUrl } }, buttons: [{ title: "게시글 보기", link: { mobileWebUrl: currentUrl, webUrl: currentUrl } }] };
    ...
    return (
        <div className="relative" ref={ref}>
            <button onClick={() => setOpen(v => !v)}
                className="flex items-center gap-1 text-xs text-slate-400 hover:text-blue-500 transition" title="공유">
            ...
            {open && (
                <div className="absolute bottom-full right-0 mb-2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-1.5 z-50">
                    <button onClick={shareKakao} className="w-full flex items-center gap-2.5 px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 transition">
                    ...
                    <button onClick={shareInstagram} className="w-full flex items-center gap-2.5 px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 transition">
                    ...
                    <div className="border-t border-gray-100 my-1" />
                    <button onClick={copyUrl} className="w-full flex items-center gap-2.5 px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 transition">
                        {copied
...
function ScrapButton({ postId, initialScrapped, initialCount, authUser }: ScrapButtonProps) {
    ...
    async function handleToggle() {
        ...
        setLoading(true);
        try {
            const data = await ajax.post(`/post/${postId}/scrap`) as { scrapped: boolean; count: number };
    ...
    return (
        <button onClick={handleToggle} disabled={loading} title={scrapped ? "스크랩 취소" : "스크랩"}
...
function LikeBar({ postId, initialLike, initialDislike, initialUserType, useLike, useDislike, authUser }: LikeBarProps) {
    ...
    async function handleToggle(type: string) {
        ...
        setLoading(true);
        try {
            const data = await ajax.post(`/post/${postId}/like`, { type }) as { like_count: number; dislike_count: number; user_type: string | null };
    ...
    return (
        <div className="flex items-center justify-center gap-3 py-5 border-t border-gray-100">
            {useLike && (
                <button onClick={() => handleToggle("LIKE")} disabled={loading}
            ...
            {useDislike && (
                <button onClick={() => handleToggle("DISLIKE")} disabled={loading}
...
interface PostDetailProps {
    post: Post & { content: string; board_name?: string; category?: string; dislike_count?: number };
    summary?: string | null;
    isOwner?: boolean;
    boardPosts?: (Post & { name?: string })[];
    boardPostsTotal?: number;
    ...
    prevPost?: Post | null;
    nextPost?: Post | null;
    seo?: SeoData & { ogImage?: string; publishedAt?: string; author?: string };
}

export default function PostDetail({
    post, summary = null, isOwner = false, boardPosts = [], boardPostsTotal = 0,
    boardPostsPage = 1, boardPostsPerPage = 15, comments = [], maxDepth = 2,
    boardOptions = {}, userLikeType = null, isScrapped = false, scrapCount = 0,
    prevPost = null, nextPost = null, seo = {}
}: PostDetailProps) {
    ...
    return (
        <ServiceLayout theme="light">
            ...
            <div className="flex items-center gap-2 mb-4 text-sm text-slate-400">
                ...
                <Link href={`/board/${post.category}`} className="hover:text-blue-500 transition">{post.board_name}</Link>
                <span className="text-slate-300">›</span>
            ...
            <article className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-5 py-4 border-b border-gray-100">
                    ...
                    <div className="flex items-center gap-3 text-xs text-slate-400">
                        ...
                        {comments.length > 0 && (
                            <><span className="text-slate-300">·</span><span>댓글 {comments.length}</span></>
                        )}
                    </div>
                </div>
                ...
                <LikeBar postId={post.post_id} initialLike={post.like_count} initialDislike={post.dislike_count}
                    initialUserType={userLikeType} useLike={boardOptions.use_like ?? true}
                    useDislike={boardOptions.use_dislike ?? false} authUser={authUser} />

                {(prevPost || nextPost) && (
                    <div className="border-t border-gray-100 text-sm">
                        {nextPost && (
                            <Link href={`/post/${nextPost.post_id}`}
                        ...
                        {prevPost && (
                            <Link href={`/post/${prevPost.post_id}`}
                ...
                <div className="px-5 py-3 border-t border-gray-100 flex items-center justify-between">
                    <Link href={`/board/${post.category}`} className="text-sm text-slate-500 hover:text-blue-500 transition">← 목록</Link>
                    <div className="flex items-center gap-4">
                        <ScrapButton postId={post.post_id} initialScrapped={isScrapped} initialCount={scrapCount} authUser={authUser} />
            ...
            {showDeleteConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                    <div className="bg-white rounded-xl shadow-xl p-6 w-80 mx-4">
                        ...
                        <p className="text-sm text-slate-500 mb-5">게시글을 삭제하면 첨부 파일과 함께 복구할 수 없습니다.<br />삭제하시겠습니까?</p>
                        <div className="flex gap-2 justify-end">
                            <button onClick={() => setShowDeleteConfirm(false)} disabled={deleting}
                                className="px-4 py-2 text-sm text-slate-600 bg-gray-100 rounded-lg hover:bg-gray-200 transition disabled:opacity-50">취소</button>
                            <button onClick={() => destroy(`/post/${post.post_id}`, { onSuccess: () => setShowDeleteConfirm(false) })}
                                disabled={deleting}
            ...
            {boardPosts.length > 0 && (() => {
                ...
                const goPage = (page: number) => router.visit(`/post/${post.post_id}`, {
                    data: { list_page: page }, preserveScroll: false, preserveState: false,
                });
                const pageNumbers = (() => {
                    const range: number[] = [];
                ...
                return (
                    <section className="mt-4 bg-white rounded-lg shadow-sm border border-gray-200">
                        <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                            ...
                            <Link href={`/board/${post.category}`} className="text-xs text-blue-500 hover:underline">전체보기</Link>
                        </div>
                                ...
                                return (
                                    <li key={p.post_id} className={isCurrent ? "bg-blue-50" : ""}>
                                        <Link href={`/post/${p.post_id}`}
                                            ...
                                            {p.is_notice && <span className="shrink-0 text-[10px] font-bold text-white bg-red-500 rounded px-1.5 py-0.5 leading-tight">공지</span>}
                                            {isCurrent && <span className="shrink-0 text-[10px] font-bold text-white bg-blue-500 rounded px-1.5 py-0.5 leading-tight">현재글</span>}
                                            <span className={`flex-1 text-sm truncate transition ${isCurrent ? "font-bold text-blue-600" : "text-slate-700 group-hover:text-blue-500"}`}>{p.title}</span>
                                            {(p.comment_count ?? 0) > 0 && <span className="shrink-0 text-xs text-blue-400 font-medium">[{p.comment_count}]</span>}
                        ...
                        {totalPages > 1 && (
                            <div className="flex items-center justify-center gap-1 px-5 py-3 border-t border-gray-100">
                                <button onClick={() => goPage(boardPostsPage - 1)} disabled={boardPostsPage <= 1}
                                    className="px-2 py-1 text-xs text-slate-500 hover:text-blue-500 disabled:opacity-30 transition">‹</button>
                                {pageNumbers.map(n => (
                                ...
                                <button onClick={() => goPage(boardPostsPage + 1)} disabled={boardPostsPage >= totalPages}
                                    className="px-2 py-1 text-xs text-slate-500 hover:text-blue-500 disabled:opacity-30 transition">›</button>
                            </div>
                        )}
                    </section>
