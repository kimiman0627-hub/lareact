...
export default function SearchResult({ query, list }: SearchResultProps) {
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-4 py-3 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                    <h1 className="text-base font-bold text-slate-800">
                        {query ? <><span className="text-blue-600">"{query}"</span> 검색 결과</> : "통합검색"}
                    </h1>
                    {list && <span className="text-xs text-slate-400">전체 {total}건</span>}
                </div>

                {!query && <div className="px-4 py-16 text-center text-sm text-slate-400">검색어를 입력해주세요.</div>}
                        ...
                        {posts.map((post) => (
                            <li key={post.post_id}>
                                <Link href={`/post/${post.post_id}`}
                                    ...
                                    <span className="shrink-0 text-[10px] font-medium text-blue-600 bg-blue-50 border border-blue-100 rounded px-1.5 py-0.5 leading-tight whitespace-nowrap">{post.board_name}</span>
                                    <span className="flex-1 text-sm text-slate-700 group-hover:text-blue-600 transition leading-snug truncate min-w-0">
                                        {post.title}
                                        {(post.comment_count ?? 0) > 0 && <span className="ml-1 text-blue-400 font-semibold text-xs">[{post.comment_count}]</span>}
                                    </span>
