...
interface AboutProps {
    stats?: { boards?: number; posts?: number };
    seo?: SeoData;
}

export default function About({ stats = {}, seo = {} }: AboutProps) {
    return (
            ...
            <div className="max-w-3xl mx-auto space-y-6">
                <div className="bg-linear-to-br from-blue-600 to-blue-800 rounded-xl px-8 py-12 text-center text-white shadow-md">
                    ...
                    <p className="text-sm text-blue-100 leading-relaxed max-w-md mx-auto">
                        국내외 다양한 커뮤니티의 인기 게시글과 실시간 금융 정보를 한 곳에서 편리하게 확인할 수 있는 통합 커뮤니티 포털입니다.
                    </p>
                    {((stats.boards ?? 0) > 0 || (stats.posts ?? 0) > 0) && (
                        <div className="flex justify-center gap-10 mt-8 pt-6 border-t border-blue-500/40">
                            {(stats.boards ?? 0) > 0 && <StatBox value={stats.boards!} label="운영 게시판" />}
                            {(stats.posts ?? 0) > 0 && <StatBox value={stats.posts!} label="누적 게시글" />}
                        </div>
                    )}
                </div>
                ...
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm px-8 py-7">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <FeatureCard icon="📰" title="인기 콘텐츠 큐레이션" desc="유머, 연예, 스포츠, 게임, IT 등 다양한 주제의 커뮤니티 인기 게시글을 실시간으로 확인할 수 있습니다." />
                        <FeatureCard icon="📈" title="실시간 금융 정보" desc="코스닥·나스닥 주식 시세, 비트코인·이더리움 가상화폐, 달러·유로·엔화 환율, 금·은 시세를 실시간으로 제공합니다." />
                        <FeatureCard icon="💬" title="커뮤니티 활동" desc="회원 가입 후 게시글 작성, 댓글, 좋아요, 스크랩 등 다양한 커뮤니티 활동에 참여하실 수 있습니다." />
                        <FeatureCard icon="🔥" title="인기글 랭킹" desc="조회수, 댓글, 좋아요를 종합한 인기 점수 기반으로 최근 7일간 가장 많이 읽힌 게시글을 확인하세요." />
                    </div>
                </div>
                    ...
                    <ol className="space-y-4">
                        {[
                            ...
                            <li key={step} className="flex gap-4">
                                <span className="shrink-0 w-8 h-8 rounded-full bg-blue-600 text-white text-xs font-black flex items-center justify-center">{step}</span>
                                <div>
                ...
                <div className="bg-slate-50 rounded-xl border border-gray-100 px-8 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <div className="flex gap-3 shrink-0">
                        <Link href="/inquiry?type=SUPPORT" className="px-4 py-2 bg-blue-600 text-white text-xs font-semibold rounded-lg hover:bg-blue-700 transition">1:1 문의</Link>
                        <Link href="/inquiry?type=PARTNERSHIP" className="px-4 py-2 bg-white border border-gray-200 text-slate-600 text-xs font-semibold rounded-lg hover:bg-gray-50 transition">제휴 문의</Link>
                    </div>
                </div>
            </div>
        </ServiceLayout>
