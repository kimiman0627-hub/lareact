...
function Ball({ number, size = "md", isBonus = false }: BallProps) {
    const sizeClass = { xs: "w-7 h-7 text-[10px]", sm: "w-8 h-8 text-xs", md: "w-10 h-10 text-sm" }[size] ?? "w-10 h-10 text-sm";
    return (
        <span className={`inline-flex items-center justify-center rounded-full font-bold text-white shadow-sm shrink-0 ${sizeClass} ${ballColor(number)} ${isBonus ? "ring-2 ring-offset-1 ring-yellow-400" : ""}`}>
            {number}
        </span>
    );
}

function formatWinAmount(amount: number | undefined): string {
    if (!amount) return "-";
...
function generatePredictions(numberFreq: Record<string, number>): number[][] {
    const all = Array.from({ length: 45 }, (_, i) => i + 1);
    if (!numberFreq || Object.keys(numberFreq).length === 0) {
        return Array.from({ length: 5 }, () => shuffle(all).slice(0, 6).sort((a, b) => a - b));
    }
    ...
    return (
        [[4, 1, 1], [1, 1, 4], [2, 2, 2], [3, 2, 1], [1, 3, 2]] as [number, number, number][]
    ).map(([hc, mc, cc]) => [
...
export default function Lotto({ seo = {} }: LottoProps) {
    const [draws, setDraws] = useState<LottoDraw[]>([]);
    const [latestDrwNo, setLatestDrwNo] = useState<number | null>(null);
    const [loading, setLoading] = useState(true);
    const [unavailable, setUnavailable] = useState(false);
    const [numberFreq, setNumberFreq] = useState<Record<string, number>>({});
    const [predictions, setPredictions] = useState<number[][]>(() => generatePredictions({}));

    useEffect(() => {
        (ajax.get("/api/lottery/lotto/results") as Promise<LottoApiResponse>)
            .then((res) => {
                const { draws: d = [], latestDrwNo: n, unavailable: u, numberFreq: freq = {} } = res;
                setDraws(d);
                setLatestDrwNo(n ?? null);
                setUnavailable(!!u);
                setNumberFreq(freq);
    ...
    return (
        <ServiceLayout>
            <SeoHead seo={seo} defaultTitle="로또 당첨번호 확인" />
            <div className="max-w-2xl mx-auto px-3 sm:px-4 py-4 sm:py-6 space-y-4 sm:space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-5">
                    <div className="flex items-center justify-between mb-3 sm:mb-4">
                        ...
                        {latest && <span className="text-xs text-gray-400">{latest.drwNoDate}</span>}
                    </div>

                    {loading && (
                        <div className="flex gap-2 justify-center py-4">
                            {[...Array(7)].map((_, i) => <div key={i} className="w-10 h-10 rounded-full bg-gray-200 animate-pulse" />)}
                        </div>
                    )}
                    {!loading && unavailable && (
                        <p className="text-center text-sm text-amber-500 py-4">동행복권 서버 점검 중입니다. 잠시 후 다시 시도해 주세요.</p>
                    )}
                    {!loading && !unavailable && latest && (
                        <>
                            <div className="flex items-center gap-1.5 sm:gap-2 justify-center flex-wrap py-1">
                                {latest.numbers.map((n) => <Ball key={n} number={n} />)}
                            ...
                            <div className="mt-3 sm:mt-4 grid grid-cols-2 gap-2 sm:gap-3">
                                <div className="bg-gray-50 rounded-lg p-3 text-center">
                                    ...
                                    <p className="text-sm sm:text-base font-bold text-blue-600">{formatWinAmount(latest.firstWinamnt)}</p>
                                </div>
                                <div className="bg-gray-50 rounded-lg p-3 text-center">
                                    <p className="text-xs text-gray-500 mb-1">1등 당첨자</p>
                                    <p className="text-sm sm:text-base font-bold text-blue-600">{latest.firstPrzwnerCo}명</p>
                                </div>
                            </div>
                        </>
                    )}
                                ...
                                {draws.map((d) => (
                                    <div key={d.drwNo} className="border border-gray-100 rounded-lg px-3 py-2.5">
                                        ...
                                        <div className="flex items-center gap-1">
                                            {d.numbers.map((n) => <Ball key={n} number={n} size="xs" />)}
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {draws.map((d) => (
                                            <tr key={d.drwNo} className="hover:bg-gray-50">
                                                <td className="py-2 pr-3 font-medium text-gray-700 whitespace-nowrap">{d.drwNo}회</td>
                                                <td className="py-2 pr-3 text-gray-400 whitespace-nowrap text-xs">{d.drwNoDate}</td>
                                                <td className="py-2">
                                                    <div className="flex items-center gap-1 flex-wrap">
                                                        {d.numbers.map((n) => <Ball key={n} number={n} size="sm" />)}
                        ...
                        {predictions.map((set, i) => (
                            <div key={i} className="flex items-center gap-2 sm:gap-3">
                                ...
                                <div className="flex gap-1 sm:gap-1.5">
                                    {set.map((n) => <Ball key={n} number={n} size="sm" />)}
                                </div>
                            </div>
                        ))}
                    </div>
                    <p className="mt-4 text-xs text-gray-300 text-center">* 예상번호는 통계 기반 참고용이며, 당첨을 보장하지 않습니다.</p>
                </div>
            </div>
        </ServiceLayout>
    );
