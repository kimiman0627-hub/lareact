...
export default function StockPage({ seo = {} }) {
        ...
        <ServiceLayout>
            ...
            <div className="max-w-2xl mx-auto px-2 sm:px-4 py-4 sm:py-6 space-y-3">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-5">
                        ...
                        <div className="flex gap-1.5">
                            {["kosdaq", "nasdaq"].map((t) => (
                                <button
                                    key={t}
                                    onClick={() => setTab(t)}
                                    className={`px-2.5 sm:px-3 py-1 rounded-lg text-xs sm:text-sm font-bold transition ${
                                        tab === t
                                            ? "bg-blue-600 text-white"
                                            : "text-slate-500 hover:text-blue-600 bg-gray-50"
                                    }`}
                                >
                                    {t.toUpperCase()}
                                </button>
                    ...
                    {/* 오류 */}
                    {!loading && error && (
                        <p className="text-center text-sm text-amber-500 py-8">
                            시세를 불러올 수 없습니다. 잠시 후 다시 시도해 주세요.
                        </p>
                    )}
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {list.map((stock, i) => (
                                    <tr key={i} className="hover:bg-gray-50">
                                        ...
                                        <td className="py-2 sm:py-2.5 pr-2">
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                <div className="min-w-0">
                                                    <p className="font-semibold text-gray-800 text-xs leading-tight truncate max-w-22.5 sm:max-w-none">
                                                        {tab === "nasdaq" ? stock.symbol : stock.name}
                                                    </p>
                                                    {tab === "nasdaq" && (
                                                        <p className="hidden sm:block text-[10px] text-gray-400 truncate sm:max-w-32.5">
                                                            {stock.name}
                                                        </p>
                                                    )}
                                                </div>
                                        ...
                                        <td className="hidden sm:table-cell py-2.5 text-right text-xs text-gray-400 whitespace-nowrap">
                                            {stock.trade_val}
                                        </td>
                                    </tr>
                                ))}
