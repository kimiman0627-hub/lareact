...
const ROLE_LABELS: Record<string, string> = { USER: "일반", ADMIN: "관리자", TEST: "테스트" };
const ROLE_COLORS: Record<string, string> = {
    USER:  "bg-blue-50 text-blue-700 border-blue-200",
    ADMIN: "bg-red-50 text-red-700 border-red-200",
    TEST:  "bg-gray-50 text-gray-500 border-gray-200",
};
const INQUIRY_STATUS: Record<string, string> = { PENDING: "대기", ANSWERED: "답변완료" };
const POST_STATUS: Record<string, string>    = { ACTIVE: "정상", INACTIVE: "비활성", DELETED: "삭제" };
...
function StatusBadge({ status, map, okKey = "ACTIVE" }: StatusBadgeProps) {
...
function SimplePager({ page, lastPage, onChange }: SimplePagerProps) {
...
function PostsTab({ userId }: PostsTabProps) {
    const [state, setState] = useState<any>({ items: [], total: 0, page: 1, lastPage: 1, loading: true });
...
// ─── 활동 탭: 댓글 ──────────────────────────────────────────────────
function CommentsTab({ userId }: CommentsTabProps) {
    const [state, setState] = useState<any>({ items: [], total: 0, page: 1, lastPage: 1, loading: true });
...
// ─── 활동 탭: 문의사항 ──────────────────────────────────────────────
function InquiriesTab({ userId }: InquiriesTabProps) {
    const [state, setState] = useState<any>({ items: [], total: 0, page: 1, lastPage: 1, loading: true });
...
function DetailModal({ userId, onClose }: DetailModalProps) {
    ...
    const tabCounts: Record<string, number> = detail ? {
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <select
                                                        ...
                                                        onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setData("user_role", e.target.value)}
                                                    >
                    ...
                    {[
                        ...
                        <div key={key}>
                            ...
                            <input type={type}
                                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                                value={data[key]}
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setData(key, e.target.value)}
                            />
...
// ─── 메인 페이지 ────────────────────────────────────────────────────
export default function UserIndex({ users, total, params }: UserIndexProps) {
    const { auth } = usePage<any>().props;

    const [startDate, setStartDate] = useState<Date | null>(params?.start_date ? new Date(params.start_date) : null);
    const [endDate,   setEndDate]   = useState<Date | null>(params?.end_date ? new Date(params.end_date) : null);
    const [searchForm, setSearchForm] = useState({
        id: params?.id || "", name: params?.name || "", email: params?.email || "",
    });

    const [createOpen,   setCreateOpen]   = useState<boolean>(false);
    const [detailUserId, setDetailUserId] = useState<any>(null);
                    ...
                    <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                        <div>
                            ...
                            <input type="number"
                                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                                value={searchForm.id}
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearchForm({ ...searchForm, id: e.target.value })}
                                placeholder="숫자 ID"
                            />
                        </div>
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearchForm({ ...searchForm, name: e.target.value })}
                                placeholder="이름 포함"
                            />
                        </div>
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearchForm({ ...searchForm, email: e.target.value })}
                                placeholder="이메일 주소"
                            />
                        </div>
                        <div>
                            ...
                            <div className="flex items-center gap-1">
                                <DatePicker selected={startDate ?? undefined} onChange={setStartDate}
                                ...
                                <DatePicker selected={endDate ?? undefined} onChange={setEndDate}
