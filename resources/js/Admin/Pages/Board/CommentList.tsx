...
/** 댓글 수정 모달 */
function EditModal({ comment, onClose }: EditModalProps) {
...
export default function CommentList({ list, total, params }: CommentListProps) {
    ...
    const comments: Comment[] = list.data ?? [];

    function search(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault();
        router.get(route("admin.comments.index"), searchForm as any, {
            preserveScroll: true,
        });
    }
    ...
    function handleModalClose(refresh: boolean) {
        setCreateOpen(false);
        setEditTarget(null);
        if (refresh) router.reload({ preserveUrl: true });
    }
