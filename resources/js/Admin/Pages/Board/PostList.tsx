...
export default function PostIndex({
    ...
    const [endDate, setEndDate] = useState<Date | undefined>(
        params?.end_date ? new Date(params.end_date) : undefined,
    );
    ...
    const allIds = list.data.map((p: any) => p.post_id);
    const allChecked = allIds.length > 0 && allIds.every((id: any) => selectedIds.has(id));
    const someChecked = !allChecked && allIds.some((id: any) => selectedIds.has(id));
    ...
    const toggleAll = () => {
        setSelectedIds((prev) => {
            ...
            if (allChecked) {
                allIds.forEach((id: any) => next.delete(id));
            } else {
                allIds.forEach((id: any) => next.add(id));
            }
            return next;
        });
    };
    ...
    const [bloggerStatus, setBloggerStatus] = useState<Record<string, string>>({});

    const publishToBlogger = useCallback(async (post: any) => {
    ...
    const openPostEditModal = async (item: any) => {
        try {
