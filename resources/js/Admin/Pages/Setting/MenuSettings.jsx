...
const newItem    = (label = "", href = "") => ({ label, href, children: [] });
const newChild   = (label = "", href = "") => ({ label, href });
...
// ── 소메뉴 행 ────────────────────────────────────────────────────────────────
function ChildRow({ child, idx, total, onChange, onRemove, onMove }) {
    return (
...
// ── 대메뉴 카드 ──────────────────────────────────────────────────────────────
function MenuCard({ item, idx, total, onChange, onRemove, onMove }) {
    const [expanded, setExpanded] = useState(true);
    const hasChildren = item.children.length > 0;

    const updateChild = (ci, field, val) => {
        const children = item.children.map((c, i) => i === ci ? { ...c, [field]: val } : c);
        onChange(idx, "children", children);
    };
    const removeChild = (ci) => onChange(idx, "children", item.children.filter((_, i) => i !== ci));
    const moveChild   = (ci, dir) => onChange(idx, "children", move(item.children, ci, dir));
...
// ── 메인 페이지 ──────────────────────────────────────────────────────────────
export default function MenuSettings({ menu: initialMenu = [] }) {
    ...
    const moveItem   = (idx, dir) => setMenu((prev) => move(prev, idx, dir));
