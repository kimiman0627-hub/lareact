...
function PublishPostModal({ onClose }: PublishPostModalProps) {
    ...
    async function selectPost(post: any) {
        ...
        setPreviewing(true);
        try {
            const data = await ajax.post("/admin/threads/preview-text", { post_id: post.post_id }) as any;
            setText(data.text ?? "");
    ...
    async function publish() {
        ...
        setResult(null);
        try {
            const data = await ajax.post("/admin/threads/publish-post", { post_id: selected.post_id, text }) as any;
            setResult(data);
            if (data.success) {
...
export default function ThreadsLogs({ logs, stats, filter, settings }: ThreadsLogsProps) {
