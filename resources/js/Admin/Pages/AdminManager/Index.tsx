...
interface AdminManagerIndexProps {
    admins: any[];
    menuItems: any[];
}
...
function Modal({ title, onClose, children, maxW = "max-w-md" }: ModalProps) {
    return (
...
function SubmitRow({ processing, label, color = "blue", onCancel }: SubmitRowProps) {
...
function CreateModal({ onClose }: CreateModalProps) {
    const form = useForm<any>({ name: "", email: "", password: "", password_confirmation: "", is_super: false });
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <Field label="비밀번호 확인" error={form.errors.password_confirmation}>
                    <input type="password" value={form.data.password_confirmation}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => form.setData("password_confirmation", e.target.value)}
                ...
                <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input type="checkbox" checked={form.data.is_super}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => form.setData("is_super", e.target.checked)}
...
/* ── 정보 수정 모달 ── */
function EditModal({ admin, onClose }: EditModalProps) {
    const form = useForm<any>({ name: admin.name, email: admin.email });
...
/* ── 비밀번호 변경 모달 ── */
function PasswordModal({ admin, currentAdminId, onClose }: PasswordModalProps) {
    const isSelf = admin.id === currentAdminId;
    const form = useForm<any>({ current_password: "", password: "", password_confirmation: "" });
                ...
                {isSelf && (
                    <Field label="현재 비밀번호" error={form.errors.current_password}>
                        <input type="password" value={form.data.current_password}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => form.setData("current_password", e.target.value)}
...
/* ── 권한 설정 모달 ── */
function PermissionsModal({ admin, menuItems, onClose }: PermissionsModalProps) {
    ...
    const selected: string[] = form.data.menu_permissions;

    function setIsSuper(val: boolean) {
        form.setData('is_super', val);
    }

    function toggleRoute(routeKey: string) {
        form.setData('menu_permissions',
            selected.includes(routeKey)
                ? selected.filter((r: string) => r !== routeKey)
                : [...selected, routeKey]
        );
    }

    function toggleGroup(routes: string[]) {
        const allChecked = routes.every((r: string) => selected.includes(r));
        form.setData('menu_permissions',
            allChecked
                ? selected.filter((r: string) => !routes.includes(r))
        ...
        <Modal title={`메뉴 권한 — ${admin.name}`} onClose={onClose} maxW="max-w-lg">
            <form onSubmit={submit} className="space-y-4">
                ...
                <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition ${isSuper ? 'bg-amber-50 border-amber-300' : 'bg-slate-50 border-gray-200 hover:bg-slate-100'}`}>
                    <input type="checkbox" checked={isSuper} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setIsSuper(e.target.checked)}
                ...
                {!isSuper && (
                    <div className="space-y-2">
                        ...
                        {menuItems.filter((item: any) => !item.super_only).map((item: any, idx: number) => {
                            const groupRoutes: string[] = item.submenu
                                ? item.submenu.map((s: any) => s.route)
                                : [item.route];
                            const allChecked  = groupRoutes.every((r: string) => selected.includes(r));
                            const someChecked = groupRoutes.some((r: string) => selected.includes(r));
                                    ...
                                    <label className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition select-none ${allChecked ? 'bg-blue-50' : 'bg-slate-50 hover:bg-slate-100'}`}>
                                        ...
                                        {someChecked && (
                                            <span className="ml-auto text-[10px] text-blue-500 font-medium">
                                                {groupRoutes.filter((r: string) => selected.includes(r)).length}/{groupRoutes.length}
                                            </span>
                                        )}
                                    </label>
...
/* ── 메인 페이지 ── */
export default function AdminManagerIndex({ admins, menuItems }: AdminManagerIndexProps) {
    ...
    const { delete: destroy, processing: deleting } = useForm<any>();
