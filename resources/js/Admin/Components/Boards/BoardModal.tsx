...
const NumberField = ({ label, value, onChange, min = 0, max }: NumberFieldProps) => (
    <div className="space-y-1">
        <label className="text-sm font-medium text-slate-600">{label}</label>
        <input
            type="number"
...
export default function BoardModal({
    ...
    const permissions: Record<string, string> = boardOptions.permissions ?? {
        ALL: "전체",
        MEMBER: "회원",
        ADMIN: "관리자",
    };
    const pointCycles: Record<string, string> = boardOptions.point_cycles ?? {
        ONCE: "최초 1회",
        DAILY: "일 1회",
        PER_POST: "작성마다",
    };
