...
const PostCreateModal = ({
    ...
    const [userInitialText, setUserInitialText] = useState<string>("");
    const [htmlMode, setHtmlMode] = useState<boolean>(false);
    const quillRef = useRef<any>(null);
    const quillKey = useRef<number>(0);
    const uploadedFileIds = useRef<any[]>([]);
    ...
    const toggleBanner = (bannerId: any) => {
        setFormData((prev: any) => {
            const ids = prev.banner_ids ?? [];
            return {
                ...prev,
                banner_ids: ids.includes(bannerId)
                    ? ids.filter((id: any) => id !== bannerId)
                    : [...ids, bannerId],
            };
        });
    };
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <div className="space-y-2">
                            ...
                            <UserSearchInput
                                ...
                                onSelect={(user: any) =>
                                    setFormData((prev: any) => ({ ...prev, user_id: user ? user.id : "" }))
                                }
                            />
                        </div>
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                type="button"
                                onClick={() => setHtmlMode((v: boolean) => !v)}
